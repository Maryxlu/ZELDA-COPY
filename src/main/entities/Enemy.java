...
public class Enemy extends Entity { // duplicate and rename this to add more different enemies
	...
	public void tick() {
		 ...
		 * Game.player.getY()) < 70)) { if(!isCollidingWithPlayer()) {
		 * //if(Game.rand.nextInt(100 ) < 50) { // apenas usar para randomizar as
		 * experiencias if((int) x < Game.player.getX() &&
		 * World.isFree((int)(x+speed),this.getY(),z) &&
		 * !isColliding((int)(x+speed),this.getY())) { x+=speed; } else if((int)x >
		 * Game.player.getX() && World.isFree((int)(x-speed),this.getY(),z) &&
		 * !isColliding((int)(x-speed),this.getY())) { x-=speed; }
		 * 
		 * if((int)y > Game.player.getY() && World.isFree(this.getX(),(int)(y-speed),z)
		 * && !isColliding(this.getX(),(int)(y-speed))) { y-=speed; } else if((int)y <
		 * Game.player.getY() && World.isFree(this.getX(),(int)(y+speed),z) &&
		 * !isColliding(this.getX(),(int)(y+speed))) { y+=speed; } } else { //we are
		 * colliding if(Game.rand.nextInt(100) < 10) { Sound.hurtSFX.play();
		 * Game.player.playerLife -= Game.rand.nextInt(5); Game.player.isDamaged = true;
		 * //System.out.println("Life: " + Game.player.playerLife);
		 * if(Game.player.playerLife <= 0) { //System.out.println("Voc� perdeu o jogo");
		 * //System.exit(1);
		 * 
		 * } } } }else { //Inactive enemy }
		 */
