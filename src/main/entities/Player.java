...
public class Player extends Entity {
	...
	public void tick() {
		...
		if (isJumping) {
			...
			if (jumpCur >= jumpFrames) {
				jumpUp = false;
				jumpDown = true;
				// System.out.println("Cehgou a altura m�xima!");
			}
		}
	...
	public void render(Graphics g) {
		if (!isDamaged) {
			...
			// g.fillRect(this.getX() - Camera.x, this.getY() - Camera.y, this.getWidth(),
			// this.getHeight());
		} else {
			if (dir == right_Dir) {
				g.drawImage(rightPlayer[index], this.getX() - Camera.x, this.getY() - Camera.y - z, null);
				if (hasGun) {
					// gun to right
					g.drawImage(Entity.WEAPON_DAMAGE_RIGHT, this.getX() + 1 - Camera.x, this.getY() + 1 - Camera.y - z,
							null);
				...
				if (hasGun) {
					// gun to left
					g.drawImage(Entity.WEAPON_DAMAGE_LEFT, this.getX() - 1 - Camera.x, this.getY() + 1 - Camera.y - z,
							null);
					g.drawImage(playerDamage, this.getX() - Camera.x, this.getY() - Camera.y - z, null);
